...
goToSlide(currentIndex)}
nextBtn.addEventListener("click",nextSlide);prevBtn.addEventListener("click",prevSlide);document.querySelectorAll(".carruselItem").forEach(item=>{const img=item.querySelector("img");const fondo=item.querySelector(".fondoBorroso");if(img&&fondo){fondo.style.backgroundImage=`url(${img.src})`}});document.querySelectorAll(".servicioFilaImagen").forEach(item=>{const img=item.querySelector("img");const fondo=item.querySelector(".fondoBorroso");if(img&&fondo){fondo.style.backgroundImage=`url(${img.src})`}});document.querySelectorAll(".carruselItem, .servicioFilaImagen, .informacionImagen").forEach(item=>{const img=item.querySelector("img");const fondo=item.querySelector(".fondoBorroso");if(img&&fondo){fondo.style.backgroundImage=`url(${img.src})`}})})()